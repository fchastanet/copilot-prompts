...
- Use the `fc-human-interaction` skill to ask for clarification.
...
Use `fc-copy-clipboard` skill to copy the improved prompt to the system clipboard, as well as typing it out in the chat.

Announce to the user that the prompt is available on the clipboard. Using `fc-human-interaction` skill ask the user if they
want any changes or additions. Repeat the copy + chat + ask after any revisions of the prompt.

Using `fc-human-interaction` skill ask the user for confirmation to apply the refined prompt. Continue this process until
