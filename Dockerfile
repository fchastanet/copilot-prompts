...
#   - STAGE my-api-builder: install all dependencies to system Python (torch, FastAPI, etc.)
#   - STAGE my-api-base: copy site-packages from my-api-builder
#     - STAGE my-api-test-runner: run tests, create test marker file
#   - STAGE my-api-dev: install dev dependencies
#   - STAGE my-api-prod: copy test marker from test-runner, copy source code, set entrypoint
...
FROM python:3.13-trixie AS my-api-builder
...
FROM python:3.13-slim-trixie AS my-api-base

SHELL ["/bin/bash", "-o", "pipefail", "-o", "errexit", "-o", "xtrace", "-c"]

# Metadata labels following OCI spec
LABEL org.opencontainers.image.title="My AI API"
...
LABEL org.opencontainers.image.source="https://github.com/CrossKnowledge/my-ai-api"
...
COPY --from=my-api-builder /usr/local/lib/python3.13/site-packages /usr/local/lib/python3.13/site-packages
COPY --from=my-api-builder /usr/local/bin /usr/local/bin
...
FROM my-api-base AS my-api-test-runner
...
FROM my-api-base AS my-api-dev
...
FROM my-api-base AS my-api-prod
...
COPY --from=my-api-test-runner /tmp/test-results/tests-passed.txt /tmp/tests-passed.txt
